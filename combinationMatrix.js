function renderCombinationMatrix({
    ...
    if (data.length != 0) {
      ...
      function CalculateTranslate(data, index, heightFirst, heightSecond) {
        var a = data.slice(0, index);
        return a.length == 0
          ? 0
          : a.length * heightFirst +
      ...
      var gRect = d3
        ...
        .attr("transform", (d, i) => {
          return `translate(${0},${
            CalculateTranslate(data, i, y_step_group, y_step) + margin.top
          })`;
        });
      ...
      var verticalLine = (d) =>
        d3.line()([
          [xLine(d), margin.top],
          [xLine(d), height - y_step + margin.top]
        ]);
