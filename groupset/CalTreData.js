function CalTreData({
    ...
    attributeSelect      // the selected attribute to be visualized as box chart or stacked chart;
} = {}) {
    ...
    dataset.map((d) => {
        ...
        for (var n = 0; n < keys.length - 1; n++) {
            if (dataFromFuzzy.find(e => e.edgeMin <= d[keys[n]] && e.edgeMax >= d[keys[n]]) != undefined) {
            ...
            if (dataFromFuzzy.find(e => e.edgeMin <= d[keys[n + 1]] && e.edgeMax >= d[keys[n + 1]]) != undefined) {
    ...
    treeDataPlus.map((d, i) => {
        ...
        // --------------------------------------------------
        // Calculate the attributes for second level
        d[1].map(
            e => {
                if (secondAggeragateAttribute == "Degree" || secondAggeragateAttribute == "Trend") {
                    e.value = JSON.parse(e[0]);
                } else {
                    e.value = e[0]
                };  // calcualte the value of value attribtue;
                ...
                // ----------------------------
                // maximum value and minimum value of second value;
                e.max = d3.max(
                    e[1]
                        .map((f) => {
                            var a = {};
                ...
                // ------------------------------------------------
                // Calcualte the selected attributes values of second level; 
                attributeSelect.map(f => {
                    var attributeName = f.name;
                    e[attributeName] = e[1].map(g => +g[attributeName])
                    e[attributeName].type = f.type;
                    e[attributeName].range = [d3.min(e[attributeName]), d3.max(e[attributeName])];
                    e[attributeName].quantile = [
                        d3.quantile(e[attributeName], 0.25),
                        d3.quantile(e[attributeName], 0.5),
                        d3.quantile(e[attributeName], 0.75)
                    ]
                })
        ...
        // --------------------------------------------------------
        // add the multiple attribute values into first level set;
        attributeSelect.map(e => {
            var attributeName = e.name;  // the name of selected attribute
            var arrayAttributeValue      // the value of selected attribute in each set, which is an array;

            d[attributeName] = d[1].map(f => f[attributeName]).reduce((a, b) => a.concat(b));
            arrayAttributeValue = d[attributeName].map(f => +f);
            d[attributeName].type = e.type;
            d[attributeName].max = d3.max(arrayAttributeValue);
            d[attributeName].min = d3.min(arrayAttributeValue);
            d[attributeName].quantile = [d3.quantile(arrayAttributeValue, 0.25),
            d3.quantile(arrayAttributeValue, 0.5),
            d3.quantile(arrayAttributeValue, 0.75)]
        })
...
// ---------------------------------------------------------------------------------------
// Add the timeTrend to dataset;
function AddtimeTrend({
    data,             // the dataset;
    dataJson,          // the dataJson;
